...
SEIRdirecttransmissioneq <- function(t, y, parms)
{
  with(
    as.list(c(y,parms)), #lets us access variables and parameters stored in y and pars by name
    {
      ...
      dE = f*S - sigma*E - n*E; #exposed
      dI = sigma*E - g*I - n*I; #infected/infectious
...
SEIRsimulate_directtransmission <- function(S0 = 1e3, E0= 0, I0 = 1, tmax = 120, scenario = 1, bd = 0.01, bf = 0, A = 1, m = 0, n = 0, g = 0.1, w = 0, sigma=0.5)
{
  ############################################################
  Y0 = c(S = S0, E = E0, I = I0, R = 0);  #combine initial conditions into a vector
  ...
  pars=c(tmax = tmax, bd = bd, bf = bf, A = A, m = m, n = n, g = g, w = w, sigma=sigma, scenario = scenario); 
  ...
  odeoutput = deSolve::lsoda(Y0, timevec, func = SEIRdirecttransmissioneq, parms=pars, atol=1e-12, rtol=1e-12);
  
  colnames(odeoutput) = c('Time','S','E', 'I','R')
