...
typhoidtransmissioneq <- function(t, y, parms)
{
  with(
    as.list(c(y,parms)), #lets us access variables and parameters stored in y and pars by name
    {
      ...
      N=S+I+Ca+R
      if (scenario==1) {f = bd*B/(B+k)}
      if (scenario==2) {f = bf*B/(B+k)}      
...
Typhoid_simulate_transmission <- function(S0 = 1e3, I0 = 1, 
                                          tmax = 120, scenario = 1, 
                                          bd = 0.01, bf = 0, 
                                          A = 1, 
                                          m = 0, 
                                          ...
                                          epsilon= 1000, delta= 1/41)
{
  ...
  pars=c(tmax = tmax, bd = bd, bf = bf, A = A, 
         m = m, n = n, 
         n_typhoid = n_typhoid,
         Ca_prop = Ca_prop,
         sigma = sigma,
