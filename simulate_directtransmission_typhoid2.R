...
typhoidtransmissioneq2 <- function(t, y, parms)
{
  with(
    as.list(c(y,parms)), #lets us access variables and parameters stored in y and pars by name
    {
      # Environmental transmission AND Human-to-Human transmission
      # force of infection for different scenarios: 
      # 1 = density-dependent, 2 = frequency-dependent
      N=S+I+Ca+R
      if (scenario==1) {f = b*B/(B+k) + bd*((I+Ca)/A)} 
      if (scenario==2) {f = b*B/(B+k) + bf*((I+Ca)/N)}      

      #the ordinary differential equations - includes birth-death and waning immunity
      dS = m*N - f*S  - n*S  + w*R;  #susceptibles 
      dI = f*S - (1-Ca_prop)*g*I - Ca_prop*sigma*I - n*I - n_typhoid*I; #infected/infectious
      dCa = Ca_prop*sigma*I - g_c*Ca #carriers
      dR =  (1-Ca_prop)*g*I + g_c*Ca - n*R - w*R;   #recovered
      dB = epsilon*(I+Ca) - delta*B  #Salmonella enterica serotype Typhi 
      #concentration in water
      list(c(dS, dI, dCa, dR, dB))
    }
...
Typhoid_simulate_transmission2 <- function(S0 = 1e3, I0 = 1, 
                                           tmax = 120, scenario = 1, 
                                           b = 0.01, bd = 0.01, bf = 0, 
                                           A = 1, 
                                           m = 0, 
                                           n = 0, 
                                           n_typhoid = 0.001,
                                           Ca_prop = 0.05, #5% become carriers
                                           sigma = 0.1,
                                           g_c = 1/3650, #0.000273973
                                           g = 0.1, w = 0,
                                           k = 10^5, 
                                           epsilon= 1000, delta= 1/41)
{
  ############################################################
  Y0 = c(S = S0, I = I0, Ca = 0, B = 0, R = 0);  #combine initial conditions into a vector
  ...
  pars=c(tmax = tmax, b = b, bd = bd, bf = bf, A = A, 
         m = m, n = n, 
         n_typhoid = n_typhoid, 
         Ca_prop = Ca_prop,
         sigma = sigma,
  ...
  odeoutput = deSolve::lsoda(Y0, timevec, 
                             func = typhoidtransmissioneq2, 
                             parms=pars, 
                             atol=1e-12, rtol=1e-12);
  
  colnames(odeoutput) = c('Time','S','I', 'Ca','R', 'B')
