...
proc import out=dat                  /*Data Name*/ 
	datafile='X:\Methods2\2024\Lab\Lab2 (week3)\mergeddata.csv'   /*Fill in XXX data location*/
	dbms=CSV replace; 
run;
...
proc univariate data=dat normal;					/* Check normality*/
	var age;										/* Mean: 17.9 year; Std Deviation: 9.5 years  */
	histogram ;				
...
proc sgplot data=dat;
  ...
  histogram pan_day;                            		/* Fill in XXX*/     
  density pan_day;										/* Normal Density with Histogram*/
  density pan_day / type=kernel;						/* Density plot (smooth out the distribution)*/
  keylegend / location=inside position=topright;
run;

/* Another approach to make histogram*/
proc univariate data=dat normal;					/* Check normality*/
	var pan_day;									/*Fill in XXX*/
	histogram ;				
...
data dat2;
	set dat2;
	pan_day4=0;									  
	if 50<= pan_day < 70 	  then pan_day4=1;
	else if 70<= pan_day < 90 then pan_day4=2;
	else if pan_day>=90 	  then pan_day4=3;
run;
...
/*siple linear*/
/*Methods 2 LAB: Lecture 3: Simple Linear Regression*
/*Two Datasets: FatherSon Data (from R) + Iris data (from SAS)*/
/*Y=beta0+beta1*X*/


/*--------------------------------------------------*/
/*Case 1: X is continuous variable (Father's Height)*/
/*--------------------------------------------------*/
/*-------------*/
/*IMPORT DATA 1*/
/*-------------*/
...
proc import out=dat                  /*Data Name*/ 
	datafile="X:\Methods2\2024\Lab\Lab2 (week3)\fatherson.csv"   /*Fill in XXX with your data location*/
	dbms=CSV replace; 
run;
proc print data=dat(obs=20);run;    /*Print firt 20 subjects*/
proc contents data=dat;run;         /*Check variable format*/
...
proc reg data=dat;					/*Y = beta0+beta1*X  */
	model son = father;				/*Please fill in Y and X*/
run;
...
proc reg data=dat;					/*Y = beta0+beta1*X  */
	model son = father;				
	output out=out1 r=res;			/*out=[residual dataset] r=residual*/
run;
...
proc glm data=dat;						/*Y = beta0+beta1*X */
	model son = father; 				/*Please fill in Y and X*/
	output out=out2 r=res;				/*out=[residual dataset] r=residual*/
run;
...
/*------------------------------------------------*/
/*Case 2: X is categorical variable (Iris Species)*/
/*------------------------------------------------*/
/*-------------*/
/*IMPORT DATA 2*/
/*-------------*/
