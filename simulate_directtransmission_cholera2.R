...
directtransmissioneq2 <- function(t, y, parms)
{
  with(
    as.list(c(y,parms)), #lets us access variables and parameters stored in y and pars by name
    {
      ...
      N=S+I+R
      if (scenario==1) {f = b*B/(B+k) + bd*(I/A)}
      if (scenario==2) {f = b*B/(B+k) + bf*(I/N)}      
      #if (scenario==1) { f=bd*I/A;}
      #if (scenario==2) { f=bf*I/N;}

      #the ordinary differential equations - includes birth-death and waning immunity
      dS = - f*S + m*N - n*S  + w*R; #susceptibles 
      dI = f*S - g*I - n*I - n_c*I; #infected/infectious
      dR = g*I -n*R           - w*R; #recovered
      dB = epsilon*I - delta*B      #V. cholerae concentration in water
      list(c(dS, dI, dR, dB))
    }
...
Cholera_simulate_directtransmission2 <- function(S0 = 1e3, I0 = 1, 
                                                 tmax = 120, scenario = 1, 
                                            b = 0.01, bd = 0.01, bf = 0, A = 1, 
                                            m = 0, n = 0, n_c = 0.001, 
                                            g = 0.1, w = 0,
                                            k = 10^5, epsilon= 10, delta= 1/41)
{
  ############################################################
  Y0 = c(S = S0, I = I0, B = 0, R = 0);  #combine initial conditions into a vector
  ...
  odeoutput = deSolve::lsoda(Y0, timevec, func = directtransmissioneq2, 
                             parms=pars, atol=1e-12, rtol=1e-12);
  
  colnames(odeoutput) = c('Time','S','I','R', 'B')
